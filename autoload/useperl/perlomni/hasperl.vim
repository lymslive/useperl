" File: hasperl
" Author: lymslive
" Description: perl omni complete with if_perl support
" Create: 2018-09-07
" Modify: 2018-09-07
...
function! s:scanModuleExportFunctions(class)
    ...
    if g:perlomni_export_functions
        ...
        let output = s:ifperl.execute(l:code)
    endif
    let l:cache = {'word': funcs, 'menu': a:class}
    return s:omnic.SetCacheNS('mef',a:class, l:cache)
endfunction
...
function! s:scanCurrentExportFunction()
    ...
    for line in lines
        ...
        if strlen(m) > 0
            cal add(funcs ,s:scanModuleExportFunctions(m))
        endif
    endfor
...
" End Load: {{{1
function! useperl#perlomni#hasperl#load() abort "{{{
    return 0
endfunction "}}}
